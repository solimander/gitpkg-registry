...
import { Network, MULTICALL, CRYPTOPUNKS, CRYPTOKITTIES, ERC1155_SUBGRAPH, ERC721_SUBGRAPH, NON_STANDARD_SUBGRAPH, NFTX_SUBGRAPH, SUSHI_SUBGRAPH, WETH_TOKEN, NFTX_MARKETPLACE_ZAP, ZEROX_URL, SUSHISWAP_ROUTER, UNISWAP_QUOTER, USDC, NFTX_STAKING_ZAP, OPENSEA_COLLECTION, NFTX_INVENTORY_STAKING } from '@nftx/constants';
...
const isCryptoPunk = address => {
  return addressEqual$1(address, CRYPTOPUNKS);
};
const isCryptoKitty = address => {
  return addressEqual$1(address, CRYPTOKITTIES);
};
...
const isStakeOrMint = (mint, network) => {
  ...
  if (addressEqual$1((_mint$user = mint.user) == null ? void 0 : _mint$user.id, getChainConstant(NFTX_STAKING_ZAP, network))) {
    // LP Stake
    return 'stake';
  }
...
const isVaultEnabled = vault => {
  ...
  if (addressEqual$1(vault.asset.id, OPENSEA_COLLECTION) && !((_vault$eligibilityMod = vault.eligibilityModule) != null && _vault$eligibilityMod.id)) {
    return false;
  }
