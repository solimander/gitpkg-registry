import { CRYPTOPUNKS, CRYPTOKITTIES, Network, ERC1155_SUBGRAPH, ERC721_SUBGRAPH, NON_STANDARD_SUBGRAPH, NFTX_SUBGRAPH, SUSHI_SUBGRAPH, WETH_TOKEN, MULTICALL, NFTX_MARKETPLACE_ZAP, ZEROX_URL, SUSHISWAP_ROUTER, UNISWAP_QUOTER, USDC, NFTX_STAKING_ZAP, OPENSEA_COLLECTION, NFTX_INVENTORY_STAKING } from '@nftx/constants';
import { BigNumber } from '@ethersproject/bignumber';
import { parseEther } from '@ethersproject/units';
import { WeiPerEther, Zero } from '@ethersproject/constants';
import * as abi$1 from '@nftx/constants/abis/NFTXMarketplaceZap.json';
import { Contract } from '@ethersproject/contracts';
import * as routerAbi from '@nftx/constants/abis/UniswapV2Router.json';
import * as abi$2 from '@nftx/constants/abis/UniswapQuoter.json';
import * as abi$4 from '@nftx/constants/abis/NFTXVaultUpgradeable.json';
import * as abi$3 from '@nftx/constants/abis/NFTXInventoryStaking.json';
import * as abi from '@nftx/constants/abis/ERC20.json';
...
const isCryptoPunk = address => {
  return (address == null ? void 0 : address.toLowerCase()) === CRYPTOPUNKS;
};
const isCryptoKitty = address => {
  return (address == null ? void 0 : address.toLowerCase()) === CRYPTOKITTIES;
};
...
const erc1155 = async _ref => {
  ...
  const assets = (_data = data) == null ? void 0 : (_data$account = _data.account) == null ? void 0 : (_data$account$balance = _data$account.balances) == null ? void 0 : _data$account$balance.map(_ref2 => {
    ...
    const quantity = Number(value) < 1 ? parseEther(value) : BigNumber.from(value);
    const asset = {
      id,
      tokenId,
      assetAddress,
...
function formatTokenReserves(token, network) {
  ...
  const weth = getChainConstant(WETH_TOKEN, network).toLowerCase(); // check to see if basePair is vtoken
  // PUNK-ETH

  if (token != null && (_token$basePairs = token.basePairs) != null && _token$basePairs.length) {
    const wethPair = token.basePairs.find(p => p.token1.id.toLowerCase() === weth);
  ...
  if (token != null && (_token$quotePairs = token.quotePairs) != null && _token$quotePairs.length) {
    const wethPair = token.quotePairs.find(p => p.token0.id.toLowerCase() === weth);
  ...
  return {
    ...
    derivedEth: token != null && token.derivedETH ? BigNumber.from(token.derivedETH) : null,
    reserveVtoken: null,
    reserveWeth: null
  };
}
...
class MulticallContract extends Contract {
...
const buyFromVault = async _ref => {
  ...
  if (['ERC721', 'ERC1155'].includes(standard) && quote == 'ETH') {
...
const fetchEthPriceFromWeb3 = async _ref2 => {
  ...
  const quote = await contract.quoteExactOutputSingle(getChainConstant(USDC, network), getChainConstant(WETH_TOKEN, network), '3000', // the fee (0.3%)
  WeiPerEther, '0' // don't care about pool limits etc
  );
  return quote;
};
...
const isStakeOrMint = (mint, network) => {
  var _mint$user, _mint$user$id, _mint$user2, _mint$user2$id, _mint$vault$inventory, _mint$vault$inventory2;

  if (((_mint$user = mint.user) == null ? void 0 : (_mint$user$id = _mint$user.id) == null ? void 0 : _mint$user$id.toLowerCase()) === getChainConstant(NFTX_STAKING_ZAP, network).toLowerCase()) {
    // LP Stake
    return 'stake';
  }

  if (((_mint$user2 = mint.user) == null ? void 0 : (_mint$user2$id = _mint$user2.id) == null ? void 0 : _mint$user2$id.toLowerCase()) === ((_mint$vault$inventory = mint.vault.inventoryStakingPool) == null ? void 0 : (_mint$vault$inventory2 = _mint$vault$inventory.id) == null ? void 0 : _mint$vault$inventory2.toLowerCase())) {
    // Inventory Stake
    return 'stake';
  }
  ...
  let mints = response.mints.flatMap(mint => {
    ...
    return mint.nftIds.map((nftId, i) => {
      ...
      return {
        ...
        ethAmount: BigNumber.from((mint == null ? void 0 : (_mint$zapAction = mint.zapAction) == null ? void 0 : _mint$zapAction.ethAmount) ?? 0),
  ...
  let redeems = response.redeems.flatMap(redeem => {
    ...
    return redeem.nftIds.map(nftId => {
      ...
      return {
        ...
        amount: 1,
        ethAmount: redeem != null && (_redeem$zapAction = redeem.zapAction) != null && _redeem$zapAction.ethAmount ? BigNumber.from(redeem.zapAction.ethAmount) : null,
  ...
  let swaps = response.swaps.flatMap(swap => {
    ...
    return swap.redeemedIds.map((nftId, i) => {
      ...
      return {
        ...
        txId: swap.id,
        amount: 1,
        ethAmount: BigNumber.from((swap == null ? void 0 : (_swap$zapAction = swap.zapAction) == null ? void 0 : _swap$zapAction.ethAmount) ?? 0),
...
const calcMidPrice = function (reserve, amount) {
  ...
  if (amount && reserve.reserveVtoken && reserve.reserveWeth) {
    return midQuote(parseEther(amount), reserve.reserveVtoken, reserve.reserveWeth);
  }
...
const calcBuyPrice = function (reserve, amount) {
  ...
  return getAmountIn(parseEther(amount), reserve.reserveWeth, reserve.reserveVtoken);
};
...
const calcSellPrice = function (reserve, amount) {
  ...
  return getAmountOut(parseEther(amount), reserve.reserveVtoken, reserve.reserveWeth);
};
...
const transformVaultReserves = reserves => {
  ...
  const enoughLiquidity = buyPrice && sellPrice ? buyPrice.sub(sellPrice).mul(WeiPerEther).div(buyPrice).lte(parseEther('0.1')) : false;
...
const transformVault = _ref => {
  ...
  const reserve = reserves.find(_ref2 => {
    let {
      tokenId
    } = _ref2;
    return tokenId === x.id;
...
const isVaultEnabled = vault => {
  ...
  if (vault.asset.id.toLowerCase() === OPENSEA_COLLECTION.toLowerCase() && !((_vault$eligibilityMod = vault.eligibilityModule) != null && _vault$eligibilityMod.id)) {
    return false;
  }
...
const doesVaultHaveTargetSwapFee = vault => {
  ...
  return ((vault == null ? void 0 : (_vault$fees = vault.fees) == null ? void 0 : _vault$fees.targetSwapFee) ?? Zero).isZero() === false;
};
const doesVaultHaveRandomSwapFee = vault => {
  var _vault$fees2;

  return ((vault == null ? void 0 : (_vault$fees2 = vault.fees) == null ? void 0 : _vault$fees2.randomSwapFee) ?? Zero).isZero() === false;
};
...
const sellIntoVault = async _ref => {
  let {
    minPrice,
    network,
    provider,
...
export { MulticallContract$1 as MulticallContract, buildWhere, buyFromVault$1 as buyFromVault, doesVaultHaveRandomSwapFee, doesVaultHaveTargetSwapFee, estimateGasAndFees$1 as estimateGasAndFees, fetch0xPrice$3 as fetch0xPrice, fetchAssetMetadata$1 as fetchAssetMetadata, fetchBuyPrice$1 as fetchBuyPrice, fetchEthPrice$1 as fetchEthPrice, fetchPool$1 as fetchPool, fetchPools$1 as fetchPools, fetchReservesForToken$1 as fetchReservesForToken, fetchReservesForTokens$1 as fetchReservesForTokens, fetchSellPrice$3 as fetchSellPrice, fetchTokenBalance$1 as fetchTokenBalance, fetchTotalSupply$1 as fetchTotalSupply, fetchUserAssets$1 as fetchUserAssets, fetchVault, fetchVaultActivity, fetchVaultBuyPrice$1 as fetchVaultBuyPrice, fetchVaultFees, fetchVaultSellPrice$1 as fetchVaultSellPrice, fetchVaultSwapPrice$1 as fetchVaultSwapPrice, fetchVaults$1 as fetchVaults, fetchXTokenShare$1 as fetchXTokenShare, fetchXTokenShares$1 as fetchXTokenShares, getContract$1 as getContract, gql, increaseGasLimit$1 as increaseGasLimit, isCryptoKitty, isCryptoPunk, isNonstandard721, isVaultSwappable, mintIntoVault$1 as mintIntoVault, parseLogEvent$1 as parseLogEvent, querySubgraph, redeemFromVault$1 as redeemFromVault, sellIntoVault$1 as sellIntoVault };
